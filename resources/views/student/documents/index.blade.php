        ...
        <div class="space-y-3">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between p-3 sm:p-4 bg-slate-50 hover:bg-slate-100 rounded-xl transition-colors space-y-2 sm:space-y-0">
                <div class="flex items-center space-x-3">
                    <div class="w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-br from-teal-400 to-cyan-500 rounded-full flex items-center justify-center text-white font-bold text-sm sm:text-base">
                        {{ substr($document->application->scholarship->name ?? 'D', 0, 1) }}
                    </div>
                    <div>
                        <p class="font-semibold text-slate-900 text-sm sm:text-base">{{ $document->name ?? 'Document' }}</p>
                        <p class="text-xs sm:text-sm text-slate-500">{{ $document->application->scholarship->name ?? 'Scholarship' }} - Uploaded {{ $document->created_at->diffForHumans() }}</p>
                    </div>
                </div>
                <div class="flex items-center space-x-3">
            ...
            <div class="text-center py-6 sm:py-8 text-slate-400">
                ...
                <p class="text-xs mt-1">Documents will appear here once you submit applications</p>
            </div>
            @endforelse
        </div>
    </div>
