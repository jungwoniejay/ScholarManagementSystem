<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl leading-tight" style="color:#e2e8f0;">Edit AI Rule</h2>
    </x-slot>
    ...
    <div class="max-w-xl mx-auto">
        ...
        <div class="rounded-2xl p-6" style="background:#0F2044;border:1px solid #1E3A8A;">
            <h1 class="text-xl font-bold mb-4" style="color:#e2e8f0;">Edit AI Rule</h1>
            <form method="POST" action="{{ route('admin.rules.update', $rule->id) }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="key" value="{{ old('key', $rule->key) }}" required
                ...
                <div>
                    ...
                    <textarea name="value" rows="4" required
                              class="w-full px-3 py-2.5 text-sm rounded-xl focus:outline-none focus:ring-2 focus:ring-yellow-500"
                              style="background:#0A1628;border:1px solid #1E3A8A;color:#e2e8f0;">{{ old('value', $rule->value) }}</textarea>
