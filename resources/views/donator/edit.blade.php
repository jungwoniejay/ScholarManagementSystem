<x-app-layout>
    <div class="px-8 py-10 max-w-3xl mx-auto">
        ...
        <form action="{{ route('donators.update', $donation->id) }}" method="POST" class="bg-white shadow rounded-2xl p-6 space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="donor_name" 
                       ...
                       value="{{ old('donor_name', $donation->donor_name) }}" required>
            </div>
            ...
            <div>
                ...
                <input type="email" name="email" 
                       class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500" 
                       value="{{ old('email', $donation->email) }}">
            </div>
            ...
            <div>
                ...
                <input type="date" name="donation_date" 
                       class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500" 
                       value="{{ old('donation_date', $donation->donation_date) }}" required>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Message</label>
                <textarea name="message" rows="3" 
                          class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500">{{ old('message', $donation->message) }}</textarea>
            </div>

            <div class="pt-4">
                <button type="submit" 
                        class="px-6 py-2 bg-blue-600 text-white font-semibold rounded-lg hover:bg-yellow-700">
                    Update Donation
                </button>
